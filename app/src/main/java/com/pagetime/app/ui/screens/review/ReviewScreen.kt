...
fun ReviewScreen(
    ...
    val intervalPreviews by viewModel.intervalPreviews.collectAsStateWithLifecycle()
                ...
                if (cards.isEmpty()) {
                    ...
                    if (revealed) {
                        RatingSection(
                            ...
                            rewardSeconds = flashcardRewardSeconds
                        )
                    }
...
private fun RatingSection(
    ...
    previews: Map<LearningRating, IntervalPreview>,
    rewardSeconds: Long
) {
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Text("How well did you remember it?", style = MaterialTheme.typography.titleMedium)
        // Anki-style 2×2 grid: two buttons per row, each with its own caption
        // stacked under the label so long captions can never overlap.
        LearningRating.entries.chunked(2).forEach { row ->
            Row(
                ...
                row.forEach { rating ->
                    ...
                    val caption = previews[rating]?.label.orEmpty()
                    Column(
        ...
        Text(
            text = if (rewardSeconds > 0) {
                "Correct answers earn ${rewardSeconds}s of browse time. \"Again\" earns nothing."
            } else {
                "Your rating changes only this card's future review time."
            },
