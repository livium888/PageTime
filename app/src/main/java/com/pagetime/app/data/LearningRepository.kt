...
class LearningRepository(
    ...
    /**
     * Simulates a review for the Anki-style interval captions on the rating
     * buttons. Pure FSRS pass — nothing is persisted, so it is safe to call
     * for all four ratings on every card reveal.
     */
    suspend fun previewNextDue(
        ...
        val existing = cardDao.getById(cardId) ?: return null
        return try {
            val oldCard = FsrsCardCodec.fromJson(existing.fsrsCardJson)
            val result = scheduler.reviewCard(oldCard, rating.toFsrs(), now, null)
            result.card().due ?: now.plus(Duration.ofDays(1))
        } catch (_: Exception) {
            // A malformed legacy card must not break the review screen; the
            // buttons simply show no caption.
            null
        }
    }
